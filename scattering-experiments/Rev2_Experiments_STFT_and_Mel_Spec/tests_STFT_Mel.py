...
def train_different_extractors(J,Q, augmentation_flag, percentage, method='STFT'):
...
# imports and defines
    ...
    def buildBaseScattering_2order(input_shape, J=J, Q=Q):
        from kymatio.numpy import Scattering1D as ScatNumpy
        from kymatio.datasets import fetch_fsdd

        scattering = ScatNumpy(J, input_shape, Q)
        ...
        x_type = tf.concat([(g1av1 - leftav1), (g2av1 - leftav1), (g3av1 - g1av1), (g4av1 - g2av1), (g5av1 - g3av1),
             (rightav1 - g4av1),(rightav1 - g5av1), (g1av2 - leftav2), (g2av2 - leftav2), (g3av2 - g1av2), (g4av2 - g2av2),
             (g5av2 - g3av2),(rightav2 - g4av2), (rightav2 - g5av2)], axis=1)
    ...
    def buildBaseScattering_hybrid(input_shape, J=J, Q=Q):
        '''
            Source: https://github.com/kymatio/kymatio/blob/master/examples/1d/classif_keras.py
        '''
    ...
    def buildBaseScattering_hybrid2(input_shape, J=J, Q=Q):
        '''
            Source: https://github.com/kymatio/kymatio/blob/master/examples/1d/classif_keras.py
        '''
        from kymatio.numpy import Scattering1D as ScatNumpy
    ...
    def build_STFT(input_shape, frame_length=256, frame_step=128):
        x = Input(shape=(input_shape,))

        # Apply STFT
        stft_output = tf.abs(tf.signal.stft(x, frame_length=frame_length, frame_step=frame_step))

        # Flatten the output
        flattened = Flatten()(stft_output)

        model = Model(inputs=x, outputs=[flattened,flattened]) # type and classification
        return model
    ...
    def build_MelSpectrogram(input_shape, frame_length=256, frame_step=128, num_mel_bins=128):
        x = Input(shape=(input_shape,))

        # Apply STFT
        stft_output = tf.signal.stft(x, frame_length=frame_length, frame_step=frame_step)

        # Convert magnitude spectrogram to Mel-Spectrogram
        num_spectrogram_bins = stft_output.shape[-1]
        mel_weights = tf.signal.linear_to_mel_weight_matrix(
            num_mel_bins=num_mel_bins,
            num_spectrogram_bins=num_spectrogram_bins,
            sample_rate=15600,
            lower_edge_hertz=0.0,
            upper_edge_hertz=1024.0
        )
        mel_spectrogram = tf.abs(tf.transpose(stft_output, perm=[0, 2, 1])) @ tf.abs(mel_weights)

        # Flatten the output
        flattened = Flatten()(mel_spectrogram)
    ...
    if method=='STFT':
        features_extract = build_STFT(X_all.shape[1])
    elif method=='Mel':
        features_extract = build_MelSpectrogram(X_all.shape[1])
    elif method=='ST1':
        features_extract = buildBaseScattering(X_all.shape[1])
    elif method=='ST2':
        features_extract = buildBaseScattering_hybrid2(X_all.shape[1],J=J,Q=Q)
    ...
    root_folder_path = '/media/everton/Dados_SATA/Downloads/Scattering_Download/Scattering_Novo/Tests_STFT_Mel' + '/results/' + method + '_P_' + str(int(percentage*100)) + '_A_' + str(augmentation_flag) + '/'
    ...
    features_extract.save(root_folder_path + 'Features_' + method + '.h5')
    ...
    for train_index, validation_index in mskf.split(X_all, strat_classes):
        ...
        x_train_type, x_train_class = features_extract.predict(x_train_class)
        x_validation_type, x_validation_class = features_extract.predict(x_validation_class)
...
parameters = {"Augmentation_flag" : [True],
              #"Percentage" : [0.75,0.5, 0.25],
              "Percentage" : [0.75],
              #"Methods": ['Mel', 'STFT'],
              "Methods": ['STFT'],
              }
...
for augmentation in parameters["Augmentation_flag"]:
    for percentage in parameters["Percentage"]:
        for method in parameters["Methods"]:
            train_different_extractors(J=10, Q=6, augmentation_flag=augmentation, method=method, percentage=percentage)
